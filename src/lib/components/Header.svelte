...
<div
	class="flex flex-row justify-between align-middle {visible
		? 'visible'
		: 'invisible'} bg-slate-200 pb-1 bg-opacity-[95%] shadow-2xl"
>
	<div class="w-1/2">
	...
	<div class="h-[50px] w-[50px] my-auto ml-auto mr-5">
		<button on:click={flipSidenavValue}>
			{#if !sidenavClicked}
				<img src={Sidenav} alt="sidenav button" />
			{:else}
				<img src={XSymbol} alt="sidenav button" />
			{/if}
		</button>
	</div>
</div>
